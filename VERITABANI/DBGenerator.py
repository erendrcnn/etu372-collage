...
tables = {
    "IDARI_PERSONEL": ["CalisanID", "Ad", "Soyad", "Adres", "Tel", "Email", "Maas", "BaslangicTarihi", "DeneyimSuresi", "Unvan"],
    "TEMIZLIK_GOREVLISI": ["CalisanID", "Ad", "Soyad", "Adres", "Tel", "Email", "Maas", "BaslangicTarihi", "DeneyimSuresi", "CalismaYeri"],
    "OGRETMEN": ["CalisanID", "Ad", "Soyad", "Adres", "Tel", "Email", "Maas", "BaslangicTarihi", "DeneyimSuresi"],
    "FULL_TIME_OGRETMEN": ["CalisanID"],
    "PART_TIME_OGRETMEN": ["CalisanID"],
    "HAFTANIN_SAATLERI": ["SaatGunID", "BaslangicVakti", "BitisVakti", "Gun"],
    "PART_TIME_OGRETMEN_MESGULIYET": ["PartTimeOgretmenMesguliyetID", "CalisanID", "SaatGunID", "Sebep"],
    "AKTIF_OGRENCI": ["OgrenciID", "Ad", "Soyad", "Numara", "Tel", "Email", "Adres", "GPA"],
    "MEZUN_OGRENCI": ["OgrenciID", "Ad", "Soyad", "Numara", "Tel", "Email", "Adres", "MezuniyetGPA", "MezuniyetTarihi"],
    "VELI": ["VeliID", "OgrenciID", "VeliAdi", "VeliSoyadi", "VeliTel", "VeliEmail"],
    "AKTIF_OGRENCI_MESGULIYET": ["AktifOgrenciMesguliyetID", "OgrenciID", "SaatGunID", "Sebep"],
    "DERS": ["DersID", "DersAdi", "DersKodu", "CalisanID"],
    "ACILAN_DERS": ["AcilanDersID", "DersID", "CalisanID", "DersKodu", "DersAdi", "Syllabus", "DersGunu", "DersSaati1", "DersSaati2"],
    "AKTIF_OGRENCI_ACILAN_DERS_ALIR": ["AktifOgrenciAcilanDersID", "AcilanDersID", "OgrenciID"],
    "AKTIF_OGRENCI_DERS_TALEP" : ["AktifOgrenciDersID", "DersID", "OgrenciID"],
    "EK_GIDER": ["GiderID", "GiderAciklamasi", "GiderMiktari", "GiderTarihi"],
    "MALZEME": ["MalzemeID", "GiderID", "MalzemeMiktari", "MalzemeFiyat", "MalzemeTuru"],
    "MALZEME_DERS": ["MalzemeDersID", "AcilanDersID", "MalzemeID"],
    "SABIT_GIDER": ["GiderID", "GiderAciklamasi", "GiderMiktari"]
}
...
def create_insert_statements(table_name, column_names, num_records):
    ...
    # PART_TIME ve FULL_TIME OGRETMEN tablolarina kayit eklerken, OGRETMEN tablosuna da kayit eklememiz gerekiyor.
    if table_name == "OGRETMEN":
        num_records *= 2
    ...
    for i in range(num_records):
        ...
        for col in column_names:
            if col == "AcilanDersID":
                ...
                if table_name == "MALZEME":
                    values.append(str(malzeme_id_counter))
                    malzeme_id_counter += 1
                elif table_name == "MALZEME_DERS":
                ...
                if table_name == "AKTIF_OGRENCI":
                    values.append(str(id_counters[col]))
                    id_counters[col] += 1
                elif table_name == "MEZUN_OGRENCI":
                ...
                if table_name == "AKTIF_OGRENCI_ACILAN_DERS_ALIR":
                ...
                if table_name == "AKTIF_OGRENCI_MESGULIYET":
                ...
                if table_name == "PART_TIME_OGRETMEN_MESGULIYET":
            ...
            # Handle other columns as before
                ...
                elif table_name == "OGRETMEN":
                    ogrt_email_list.append(email)
                elif table_name == "IDARI_PERSONEL":
                    idari_email_list.append(email)
                elif table_name == "VELI":
                    veli_email_list.append(email)
                elif table_name == "TEMIZLIK_GOREVLISI":
...
table_dependencies = {
    ...
    "TEMIZLIK_GOREVLISI": [],
    "OGRETMEN": [],
    "PART_TIME_OGRETMEN": ["OGRETMEN"],
    "FULL_TIME_OGRETMEN": ["OGRETMEN"],
    "HAFTANIN_SAATLERI": [],
    "PART_TIME_OGRETMEN_MESGULIYET": ["PART_TIME_OGRETMEN", "HAFTANIN_SAATLERI"],
    "MEZUN_OGRENCI": [],
    "AKTIF_OGRENCI": [],
    "VELI": ["AKTIF_OGRENCI"],
    "AKTIF_OGRENCI_MESGULIYET": ["AKTIF_OGRENCI", "HAFTANIN_SAATLERI"],
    "DERS": ["OGRETMEN"],
    "ACILAN_DERS": ["OGRETMEN", "DERS"],
    "AKTIF_OGRENCI_ACILAN_DERS_ALIR": ["ACILAN_DERS", "AKTIF_OGRENCI"],
    "AKTIF_OGRENCI_DERS_TALEP" : ["DERS", "AKTIF_OGRENCI"],
    "EK_GIDER": [],
    "MALZEME": ["EK_GIDER"],
    "MALZEME_DERS": ["ACILAN_DERS", "MALZEME"],
    "SABIT_GIDER": [],
}
...
# Sonrasinda HAFTANIN_SAATLERI tablosu icin INSERT kodlarini yazdiralim.
print("-- Insert statements for HAFTANIN_SAATLERI")
for gun in gunler:
    for saat_araligi in saat_araliklari:
        print(f"INSERT INTO HAFTANIN_SAATLERI (SaatGunID, BaslangicVakti, BitisVakti, Gun) VALUES ({saat_gun_id_counter}, {saat_araligi}, '{gun}');")
        output_file.write("INSERT INTO HAFTANIN_SAATLERI (SaatGunID, BaslangicVakti, BitisVakti, Gun) VALUES (" + str(saat_gun_id_counter) + ", " + saat_araligi + ", '" + gun + "');\n")
        saat_gun_id_counter += 1
print()
output_file.write("\n")
...
for table_name, column_names in tables.items():
    ...
    if table_name != "HAFTANIN_SAATLERI":
...
# ÖĞRENCİLERİN MEŞGUL OLMADIĞI YERLERE DERS SAATLERİNİN DEĞERLERİNİ ATAYACAK ŞEKİLDE BİR QUERY YAZ.
print("-- Update statements for ACILAN_DERS")
print("UPDATE ACILAN_DERS ad\n" +
      "SET ad.DersSaati = (\n" +
      "    SELECT h.BaslangicVakti\n" +
      "    FROM HAFTANIN_SAATLERI h\n" +
      "    LEFT JOIN PART_TIME_OGRETMEN_MESGULIYET ptm ON h.SaatGunID = ptm.SaatGunID\n" +
      "    LEFT JOIN AKTIF_OGRENCI_MESGULIYET aom ON h.SaatGunID = aom.SaatGunID\n" +
      "    WHERE ptm.CalisanID IS NULL AND aom.OgrenciID IS NULL\n" +
      "    ORDER BY RAND()\n" +
      "    LIMIT 1\n" +
      ");")
output_file.write("UPDATE ACILAN_DERS ad SET ad.DersSaati = (SELECT h.BaslangicVakti FROM HAFTANIN_SAATLERI h LEFT JOIN PART_TIME_OGRETMEN_MESGULIYET ptm ON h.SaatGunID = ptm.SaatGunID LEFT JOIN AKTIF_OGRENCI_MESGULIYET aom ON h.SaatGunID = aom.SaatGunID WHERE ptm.CalisanID IS NULL AND aom.OgrenciID IS NULL ORDER BY RAND() LIMIT 1);" + "\n")
